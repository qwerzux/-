...
def menu_operators():
    while True:
        ...
        if choice == "1":
            ...
            operator = Operator(full_name=full_name, position=position, phone=phone)
            operator.save()
            ...
            if operator:
                ...
                full_name = input(f"ФИО [{operator.full_name}]: ")
                position = input(f"Должность [{operator.position}]: ")
                phone = input(f"Телефон [{operator.phone}]: ")

                operator.full_name = full_name if full_name else operator.full_name
                operator.position = position if position else operator.position
                operator.phone = phone if phone else operator.phone

                operator.save()
                print("Данные оператора изменены.")
            else:
                print("Оператор не найден.")
